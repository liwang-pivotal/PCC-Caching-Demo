...
/**
 * Reused from https://github.com/gemfire/cf-gemfire-connector-examples
 *
 * @author Pulkit Chandra
 */
public class EnvParser {
	private final static Logger logger = LoggerFactory
			.getLogger(EnvParser.class);
		...
		for (String locator : locators) {
			...
			if (!m.matches()) {
				throw new IllegalStateException(
						"Unexpected locator format. expected host[port], got"
								+ locator);
			}
			locatorList.add(new URI("locator://" + m.group(1) + ":"
					+ m.group(2)));
		}
		return locatorList;
	}
	...
	private List getGemFireService(Map services) {
		...
		if (l == null) {
			throw new IllegalStateException(
					"GemFire service is not bound to this application");
		}
		return l;
	}
}
